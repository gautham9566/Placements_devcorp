...
export async function POST(request) {
  try {
    ...
    const response = await fetch(`${ENGAGEMENT_SERVICE_URL}/dislikes`, {
      method: 'POST',
      headers: {
...
export async function DELETE(request) {
  try {
    ...
    const response = await fetch(
      `${ENGAGEMENT_SERVICE_URL}/dislikes?lms_username=${lms_username}&content_type=${content_type}&content_id=${content_id}`,
      {
        method: 'DELETE',
      }
    );
